...
![Visualization](Figure1.png)

We see that the Black-Scholes hedging PnL is relatively more spread out, with a much higher variance than the model-based hedging. The model's expected PnL is also slightly better than the theoretical. The next question would be if we could improve the model's distribution so as to have a comparable number of red and blue bars to the right of zero, where currently, the red bars are higher.
